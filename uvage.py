...
import sys, urllib, os.path, pygame
...
def from_text(x, y, text, fontsize, color, font_name=None, bold=False, italic=False):
...
class Camera(object):
    ...
    def __init__(self, width, height, full_screen=False):
        ...
        self.y = screen_height / 2 + screen_height

        self.is_transitioning = True
        self.finished_transition = False

    def set_pos(self, player_x_pos, player_y_pos):
        if not self.editing_mode:
            self.x = player_x_pos
            self.y = player_y_pos - 176
        else:
            self.x += self.x_vel
            self.y += self.y_vel
    ...
    def display(self):
        ...
        if self.is_transitioning and not self.finished_transition:
            if self.y > screen_height / 2 + self.speed:
                self.y -= self.speed
            else:
...
keys = set([])
keys_keydowned_this_frame = set([])

def is_pressing(key):
    '''Returns a boolean that represents whether the given computer key is being pressed.'''
    global keys
    if key.value not in key_constants:
        raise KeyError("Key name " + key.value + " is not a valid key name.")
    elif key_constants[key.value] in keys:
        return True
    else:
        return False

def did_keydown(key):
    global keys_keydowned_this_frame
    if key.value not in key_constants:
        raise KeyError("Key name " + key.value + " is not a valid key name.")
    elif key_constants[key.value] in keys_keydowned_this_frame:
        return True
    else:
        return False
...
def timer_loop(fps, callback, limit=None):
    ...
    ----'''
    global _timeron, _timerfps, keys, keys_keydowned_this_frame
    keys = set([])
    keys_keydowned_this_frame = set([])
    if fps > 60: fps = 60
    _timerfps = fps
    ...
    while not limit or frames < limit:
        ...
        if event.type == pygame.KEYDOWN:
            if event.key not in keys:
                keys_keydowned_this_frame.add(event.key)
            keys.add(event.key)
